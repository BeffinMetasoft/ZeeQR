...
import addIcon from '../../../assests/img1/Vector.svg';
...
function ClassicNewTheme({ card }) {
    ...
    return (
        <div>
            <section className="previewWrap1New " style={{ backgroundImage: `url(${card.bgImg ? card.bgImg : background})`, backgroundSize: '100%', backgroundPosition: 'center 150px', backgroundColor: `${card.bgImg ? '#0a0f23' : ''}` }}>
                ...
                <div className="previewContainer1New">
                    <div className="header"></div>
                    {card.checkPfCard ?

                        <div className={`userDetails ${!card.checkProfile ? "flex flex-col jutify-center" : ''}`} style={{ backgroundColor: `${card.colorCode ? card.colorCode : 'white'}` }} >
                            {!card.checkProfile ? '' :
                            ...
                            <div className="user " style={{ width: `${card.checkProfile ? 'calc(100% - 122px)' : ''}` }}>
                                ...
                                <h1 style={{ width: `${!card.checkProfile ? "max-content" : ''}`, fontSize: `${array[0]?.length > 12 || array[1]?.length > 12 || array[2]?.length > 12 ? '20px' : ''}`, color: `${card.colorCode ? 'white' : ''}` }}  >{card.name}</h1>

                                <p style={{ color: `${card.colorCode ? 'white' : ''}` }}>{card.companyDesignation}</p>
                            </div>
                        </div> : ''
                    }
                    <div className="social-links" style={{ backgroundColor: `${card.colorCode ? card.colorCode : ''}` }} >
                        <Link onClick={(e) => {
                            window.location = `mailto:${card.email}`;
                            e.preventDefault();
                        }} target="_blank" style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }}>
                            <img src={mailiconw} alt='' />
                        </Link>
                        <Link to={`tel:+${card.phone}`} style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }} >
                            <img src={phoneiconw} alt='' />
                        </Link>
                        <Link to={`sms:+${card.phone}`} style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }} >
                            <img src={messageiconw} alt='' />
                        </Link>
                        <Link to={`https://wa.me/+${card.whatsappNumber}?text=Hi%2C`} target="_blank" style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }} >
                    ...
                    <div className="contactOptions" style={{ backgroundColor: `${card.colorCode ? card.colorCode : 'white'}`, color: `${card.colorCode ? 'white' : ''}` }}>
                        <h2 ><span style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }}><img src={phoneiconw} alt='' /></span> Contact Me</h2>
                        <h3>Call</h3>
                        ...
                        {card.locationUrl ?
                            <Link target='_blank' to={`${card.locationUrl}`} className='blk-btn' style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }} ><img src={arrowIcon} alt='' />Direction</Link>
                            : ""
                        }
                    </div>
                        ...
                        <div className="contactOptions" style={{ backgroundColor: `${card.colorCode ? card.colorCode : 'white'}` }}>
                            ...
                            <div className="social-media" style={{ backgroundImage: `url(${card.colorCode ? arrow_white : arrow})`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right' }}>
                                <Link to={`${card.websiteUrl}`} target="_blank" >
                                    <img className='rounded-full p-2 '  src={webIcon} alt='' style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : 'black'}` }} />
                    ...
                    <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none" >
                        <div className="relative w-auto my-6 mx-auto max-w-3xl">
                            ...
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-transparent outline-none focus:outline-none">
                                ...
                                {/*body*/}
                                <div className="popup-containerNew justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50  outline-none focus:outline-none" id="modal" >
                                    ...
                                    <div className="userDetails">
                                        ...
                                        {!showResults ?
                                            <div className='addToHomeNew' onClick={onClick} style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }}>Add to Home Screen</div>
                                            :
                                            <img className='addtoHomeImageNew' src={addtoHome} alt='' />}

                                    </div>

                                    <button className="cursor-pointer absolute top-0 left-0 mt-6 close-popup  ml-5 text-black hover:text-gray-600 transition duration-150 ease-in-out rounded focus:ring-2 focus:outline-none focus:ring-gray-600" onClick={handleClose} style={{ backgroundColor: `${card.iconBgColor ? card.iconBgColor : ''}` }} >
...
export default ClassicNewTheme